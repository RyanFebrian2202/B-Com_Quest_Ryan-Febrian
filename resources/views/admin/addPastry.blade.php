    ...
    <div class="container">
        <div class="row">
            ...
            <div class="form-bg col-md-6 p-4 rounded-3">
                <h1 style="font-weight: bold; display: flex; justify-content: center; align-self: center">Add Pastry
                </h1>
                <br>

                <form action="{{ route('addPastry')}}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="mb-3">
                        ...
                        <input name='quantity' type="numeric" class="form-control" id="formGroupExampleInput"
                            placeholder="Input pastry quantity">
                    </div>
                    @error('quantity')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
                    <div class="mb-3">
                        ...
                        <input name='price' type="numeric" class="form-control" id="formGroupExampleInput"
                            placeholder="Input pastry price">
                    </div>
                    @error('price')
                    <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
